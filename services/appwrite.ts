...
// Plattformspezifische SDK-Importe:
// 1)Web-Vercel -> das Paket appwrite - Web SDK, unterstützt CORS im Browser
// 2) Mobilgeräten -> react-native-appwrite – das native SDK mit setPlatform
let Client: any,
  Account: any,
  Avatars: any,
  Databases: any,
...
if (Platform.OS === "web") {
  ...
  const rna = require("react-native-appwrite");
  Client = rna.Client;
  Account = rna.Account;
  Avatars = rna.Avatars;
  Databases = rna.Databases;
  ID = rna.ID;
  Query = rna.Query;
}
...
const COLLECTION_USERS_ID =
  process.env.EXPO_PUBLIC_APPWRITE_COLLECTION_USER_ID!;
...
// Appwrite-Dienste initialisieren
...
//die Liste aller gespeicherten Filme für einen bestimmten Benutzer
